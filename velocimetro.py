...
def main() -> None:
    ...
    with tab4:
        ...
        taxas_medias = {}
        for label, dados in conv_mensal.items():
            taxas_medias[label] = dados["media"] / 100.0 if dados["media"] > 0 else 0.0

        t_ag_ven = max(0.005, taxas_medias.get("Agendamentos → Vendas", 0.05))
        t_vi_ven = max(0.01, taxas_medias.get("Visitas → Vendas", 0.10))
        t_pas_ven = max(0.02, taxas_medias.get("Pastas → Vendas", 0.20))
        t_pa_ven = max(0.05, taxas_medias.get("Pastas Aprovadas → Vendas", 0.40))

        metas_dict = {
            "vendas": meta_vendas_mes,
            "pastas_aprovadas": meta_vendas_mes / t_pa_ven,
            "pastas": meta_vendas_mes / t_pas_ven,
            "visitas": meta_vendas_mes / t_vi_ven,
            "agendamentos": meta_vendas_mes / t_ag_ven,
        }
        ...
        for etapa in FUNIL_ETAPAS:
            ...
            real_dia_lista, real_acum_lista = [], []
            soma_parcial = 0.0
            for d in df_p["dia"]:
                val_dia = float(sub_real[sub_real["dia_mes"] == d][etapa].sum()) if not sub_real.empty else 0.0
                if eh_mes_atual and d > dia_limite:
                    real_dia_lista.append(np.nan)
                    real_acum_lista.append(np.nan)
                else:
                    real_dia_lista.append(val_dia)
                    soma_parcial += val_dia
                    real_acum_lista.append(soma_parcial)
            df_p["realizado_diario"] = real_dia_lista
            df_p["realizado_acum"] = real_acum_lista

            fig = make_subplots(rows=1, cols=2, subplot_titles=("Fluxo Diário", "Volume Acumulado"))
            
            # Diário
            fig.add_trace(go.Scatter(x=df_p["dia"], y=df_p["realizado_diario"], mode="lines+markers", name="Realizado (Dia)", line=dict(color=COR_AZUL_ESC, width=2)), row=1, col=1)
            fig.add_trace(go.Scatter(x=df_p["dia"], y=df_p["meta_diaria"], mode="lines+markers", name="Meta (Dia)", line=dict(color=COR_VERMELHO, width=2, dash="dash")), row=1, col=1)
            
            # Acumulado
            fig.add_trace(go.Scatter(x=df_p["dia"], y=df_p["realizado_acum"], mode="lines+markers", name="Realizado (Acum.)", line=dict(color=COR_AZUL_ESC, width=3)), row=1, col=2)
            fig.add_trace(go.Scatter(x=df_p["dia"], y=df_p["meta_acum"], mode="lines+markers", name="Meta (Acum.)", line=dict(color=COR_VERMELHO, width=2, dash="dash")), row=1, col=2)

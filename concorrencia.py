...
def geocode_address_precision(address: str) -> tuple[Optional[float], Optional[float]]:
    ...
    try:
        geolocator = Nominatim(user_agent="direcional_precision_v6")
...
def load_base_temporal() -> dict[str, pd.DataFrame]:
    import gspread
    ...
    for ws in sh.worksheets():
        if ws.title in ["BD TEMPORAL", "DADOS DIRECIONAL"]:
...
def process_pipeline():
    ...
    # VGV Rate: VGV Vendido / VGV Total (Vendido + Potencial Estoque)
...
def main():
    ...
    st.markdown("<div style='text-align: center; font-weight: bold; margin-bottom: 1rem;'>Parâmetros do Cluster Regional</div>", unsafe_allow_html=True)
    ...
    # -------------------------------------------------------------------------
    # Geocodificação e Filtro de Raio
    # -------------------------------------------------------------------------
    # Obter dados do alvo para centralizar
    df_alvo_info = df_master[df_master["EMPREENDIMENTO"] == alvo_selecionado].iloc[0]
    endereco_alvo = df_alvo_info["ENDEREÇO"]
    regiao_alvo = df_alvo_info["REGIÃO"]
    chave_alvo = df_alvo_info["CHAVE"]
    
    with st.spinner("Sincronizando localizações do cluster..."):
        lat_alvo, lon_alvo = geocode_address_precision(endereco_alvo)
        if not lat_alvo:
            st.error("Endereço central não localizado. Verifique a coluna ENDEREÇO na base.")
            return

        # Filtrar Cluster por Região primeiro
        df_reg = df_master[df_master["REGIÃO"] == regiao_alvo].copy()
        
        # Geocodificar vizinhos únicos
        addrs_u = df_reg["ENDEREÇO"].dropna().unique()
        coords_cache = {addr: geocode_address_precision(addr) for addr in addrs_u}
        
        df_reg["lat"] = df_reg["ENDEREÇO"].map(lambda x: coords_cache.get(x, (None, None))[0])
        df_reg["lon"] = df_reg["ENDEREÇO"].map(lambda x: coords_cache.get(x, (None, None))[1])
    ...
    selected_emps = st.multiselect(
        "Selecione Empreendimentos para comparação detalhada nos gráficos",
        cluster_names,
        default=[alvo_selecionado] + cluster_names[:2] if len(cluster_names) > 1 else [alvo_selecionado]
    )

    # -------------------------------------------------------------------------
    # KPIs Comparativos (Mês Selecionado)
    # -------------------------------------------------------------------------
    ...
    estoque_total = df_mes["ESTOQUE"].sum()
    ...
    st.markdown(f"""
        <div class="vel-kpi-row">
            ...
            <div class="vel-kpi"><div class="lbl">Estoque no Raio</div><div class="val">{int(estoque_total)} unid.</div></div>
            <div class="vel-kpi"><div class="lbl">Empreendimentos no Raio</div><div class="val">{df_mes["CHAVE"].nunique()}</div></div>
        </div>
    """, unsafe_allow_html=True)
    ...
    if selected_emps:
        ...
        with g1:
            st.markdown("**Taxa de Absorção por Empreendimento**")
        ...
        with g2:
            ...
            st.markdown("**Variação de Preço Médio**")
